...
public class ExerciseLibraryUI extends JPanel {
    ...
    private void renderNormalMode() {
        if (library.getLib() == null || library.getLib().isEmpty()) return;

        cardsContainer.add(Box.createRigidArea(new Dimension(0, 8)));
        for (Exercise ex : library.getLib()) {
            if (activeFilter.equals("TẤT CẢ") || ex.getCategory().name().equals(activeFilter)) {
    ...
    private void renderSuggestMode() {
        ...
        JLabel lblMsg = new JLabel("<html><div style='text-align: center; width: 300px;'>Danh sách đề xuất theo thuật toán cho mục tiêu <b>" + u.getGoal() + "</b>:</div></html>");
        ...
        List<Exercise> filteredSuggestions = currentSuggestions.stream()
                .filter(ex -> activeFilter.equals("TẤT CẢ") || ex.getCategory().name().equals(activeFilter))
                .collect(Collectors.toList());

        if (filteredSuggestions.isEmpty()) {
             JLabel lblEmpty2 = new JLabel("Không có bài tập nào thuộc thể loại " + activeFilter + " trong danh sách đề xuất.");
        ...
        ExerciseCategory[] displayOrder = {
            ExerciseCategory.COMPOUND, 
            ExerciseCategory.ISOLATE, 
            ExerciseCategory.CARDIO, 
            ExerciseCategory.FLEXIBILITY
        };

        for (ExerciseCategory cat : displayOrder) {
            List<Exercise> list = groupedExercises.get(cat);
            if (list != null && !list.isEmpty()) {
                JLabel lblHeader = new JLabel(cat.name() + " (" + list.size() + " bài)");
