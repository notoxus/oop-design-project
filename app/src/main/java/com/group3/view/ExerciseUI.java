...
public class ExerciseUI extends JPanel {
	...
	public ExerciseUI(WorkoutLogController logController, WorkoutHandling handling, User user, DashboardUI dashboard) {
	...
	private void initComponents() {
		...
		exerciseCard.setBorder(
				BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(33, 150, 243), 1, true),
		...
		gbc.gridwidth = 1;
		lblWeight = new JLabel("Mức tạ (kg):");
		txtWeight = new HintTextField(10);
		lblReps = new JLabel("Số hiệp (reps):");
		txtReps = new HintTextField(10);
		lblDistance = new JLabel("Quãng đường (km):");
		txtDistance = new HintTextField(10);
		lblTime = new JLabel("Thời gian (phút):");
		txtTime = new HintTextField(10);

		gbc.gridy = 1;
		gbc.gridx = 0;
		formPanel.add(lblWeight, gbc);
		gbc.gridx = 1;
		formPanel.add(txtWeight, gbc);
		gbc.gridy = 2;
		gbc.gridx = 0;
		formPanel.add(lblReps, gbc);
		gbc.gridx = 1;
		formPanel.add(txtReps, gbc);
		gbc.gridy = 3;
		gbc.gridx = 0;
		...
		btnSave.addActionListener(e -> {
			try {
				...
				WorkoutLog log = new WorkoutLog.WorkoutLogBuilder().setUserID(user.getUserID())
						...
						.setExercise(currentExercise).setWeight(weight).setReps(reps).setDistance(distance)
						.setTime(time).build();

				if (logController.addWorkoutLog(log)) {
					JOptionPane.showMessageDialog(this, "Đã lưu log thành công!", "Thành công",
	...
	private Double getDoubleValue(HintTextField txt) {
		...
		if (isHintEnabled && !txt.getHint().isEmpty())
			return Double.parseDouble(txt.getHint());
		return null;
	}
	...
	private Integer getIntValue(HintTextField txt) {
		...
		if (isHintEnabled && !txt.getHint().isEmpty())
			return Integer.parseInt(txt.getHint());
		return null;
	}
	...
	private void updateFormVisibility(Exercise ex) {
		...
		lblWeight.setVisible(showWeight);
		txtWeight.setVisible(showWeight);
		lblReps.setVisible(showReps);
		txtReps.setVisible(showReps);
		lblDistance.setVisible(showDist);
		txtDistance.setVisible(showDist);
		lblTime.setVisible(showTime);
	...
	private void updateHints() {
		...
		WorkoutLog lastLog = getLastLogOfCurrentExercise(logs);
		if (lastLog == null) {
	...
	private WorkoutLog getLastLogOfCurrentExercise(List<WorkoutLog> logs) {
