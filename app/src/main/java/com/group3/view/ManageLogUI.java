...
public class ManageLogUI extends JPanel implements Observer {
	...
	private JButton makeTab(String label, boolean active) {
		...
		if (active)
			btn.setBorder(BorderFactory.createMatteBorder(0, 0, 2, 0, new Color(33, 150, 243)));
		else
	...
	private void loadWorkoutData() {
		...
		for (WorkoutLog log : logs) {
			...
			workoutModel.addRow(
					new Object[] { log.getDate().format(formatter), log.getExercise().getExerciseName(), resultStr });
		}
	}
	...
	private void loadNutritionData() {
		...
		for (NutritionLog log : logs) {
			...
			nutritionModel.addRow(new Object[] { log.getAddTime().format(formatter), log.getProductName(),
	...
	private void deleteWorkoutLog() {
		...
		if (row == -1) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn log cần xóa!");
			return;
		}
		if (displayedWorkoutLogs == null || row >= displayedWorkoutLogs.size())
			return;
	...
	private void deleteNutritionLog() {
		...
		if (displayedNutritionLogs == null || row >= displayedNutritionLogs.size())
			return;
