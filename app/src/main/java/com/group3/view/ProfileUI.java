...
public class ProfileUI extends JPanel {
	...
	public ProfileUI(User currentUser, MainFrame mainFrame) {
		...
		String[][] rows = { { "👤", "Họ và tên", currentUser.getName() != null ? currentUser.getName() : "—" },
		...
		for (String[] row : rows)
			infoCard.add(makeInfoRow(row[0], row[1], row[2]));
		...
		btnLogout.addMouseListener(new MouseAdapter() {
			public void mouseEntered(MouseEvent e) {
				btnLogout.setBackground(DANGER_D);
			}
	...
	private JPanel makeStatCard(String label, String value) {
		...
		card.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createLineBorder(new Color(230, 230, 230), 1, true), new EmptyBorder(10, 6, 10, 6)));
